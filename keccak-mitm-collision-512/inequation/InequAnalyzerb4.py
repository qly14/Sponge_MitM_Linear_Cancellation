				...
				for x4 in {0,1}:
					for x5 in {0,1}:
						for x6 in {0,1}:
							for x7 in {0,1}:
								for x8 in {0,1}:
									space.append((x1,x2,x3,x4,x5,x6,x7,x8))
	return set(space)



def build_Impossible():
	points = buildWholeVectorSpace() - AllPossible
	result = []
	for x in points:
		if (not isEqual(x)):
			result.append(x)
	return (points-set(result))
	#return (buildWholeVectorSpace() - AllPossible)
...
def isSat(point, ineq):
	...
	temp = point[0]*ineq[0] + point[1]*ineq[1] + point[2]*ineq[2] + point[3]*ineq[3] + point[4]*ineq[4] + point[5]*ineq[5] + point[6]*ineq[6] + point[7]*ineq[7] +ineq[8]
	return ( temp >= 0)

def isEqual(point):
	#temp = point[4] + point[5] - point[6] - point[7] + point[8] + point[9] - point[10]
	temp1 = point[0] + point[3]
	temp2 = point[1] + point[2]
	temp3 = point[0] + point[1]
	temp4 = point[2] + point[3]
	return ( (temp1 >= 1) & (temp2 >= 1) & (temp3 >= 1) & (temp4 >= 1) )
...
if __name__ == "__main__":
	Ineqs = convexHullParser('linear_condb4.txt')
